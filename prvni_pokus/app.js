...
function saveQueueToStorage() { localStorage.setItem('offlineQueue', JSON.stringify(offlineQueue)); }

// --- TOAST ---
function showToast(message, type='success'){
    const existing = document.getElementById('toast'); if(existing) existing.remove();
    const toast = document.createElement('div'); toast.id='toast'; toast.textContent=message; toast.className=`toast toast-${type}`;
...
// --- JSONBIN HELPERS ---
async function readDB(){ const res=await fetch(JSONBIN_URL+'/latest',{headers:{'X-Master-Key':JSONBIN_API_KEY}}); if(!res.ok) throw new Error('Chyba cteni DB:'+res.status); const json=await res.json(); return json.record; }
async function writeDB(data){ const res=await fetch(JSONBIN_URL,{method:'PUT',headers:{'Content-Type':'application/json','X-Master-Key':JSONBIN_API_KEY},body:JSON.stringify(data)}); if(!res.ok) throw new Error('Chyba zapisu DB:'+res.status); }
...
async function safeWriteScan(scanEntry){
    for(let attempt=0;attempt<MAX_RETRIES;attempt++){
        try{
            ...
            await writeDB(db);
            return true;
        }catch(err){ if(attempt<MAX_RETRIES-1) await new Promise(r=>setTimeout(r,RETRY_DELAY_MS)); }
    } return false;
}
...
async function flushOfflineQueue(){
    ...
    const toRetry=[...offlineQueue]; offlineQueue=[]; saveQueueToStorage();
    let failed=[];
    for(const entry of toRetry){ const ok=await safeWriteScan(entry); if(!ok) failed.push(entry); }
...
function updateUIForRole(){ teacherNav.style.display = (currentUser==='teacher')?'block':'none'; }
...
async function loadHistory(){
    const list=document.getElementById('historyList'); list.innerHTML='<li>Nacitam...</li>';
    const db=await readDB(); list.innerHTML='';
    const userScans=(db.scans||[]).filter(d=>d.user===currentUser);
    userScans.forEach(item=>{ const li=document.createElement('li'); li.textContent=`${item.time} – ${item.data}`; list.appendChild(li); });
}

// --- TEACHER DASHBOARD ---
async function loadTeacherDashboard(){
    const list=document.getElementById('teacherList'); list.innerHTML='<li>Nacitam...</li>';
    const db=await readDB(); list.innerHTML='';

    // vykresli vsechny uzivatele
    ctxMap.clearRect(0,0,mapCanvas.width,mapCanvas.height);
    ctxMap.drawImage(mapImage,0,0,mapCanvas.width,mapCanvas.height);
...
document.getElementById('createRunBtn').addEventListener('click', async ()=>{
    if(!currentUser||currentUser!=='teacher'){ showToast('Pouze učitel může vytvořit běh','error'); return; }
    const db = await readDB();
    db.scans = []; db.users = {};
    await writeDB(db);
...
// --- ONLOAD ---
window.addEventListener('load',()=>{ if(navigator.onLine && offlineQueue.length>0){ flushOfflineQueue(); } });
